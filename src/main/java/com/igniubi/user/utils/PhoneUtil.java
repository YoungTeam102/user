...
public class PhoneUtil {
	...
	public static boolean areaMobileVerify(String area, String mobile){
		...
		if(arr.length == 1 && StringUtils.isEmpty(area) ) {
			area = "86";
...
//		String mobile = "111111111112";
//		boolean a = isMobile(mobile);
//		System.out.print(a);
//	}
//判断手机号是否是以147,170,171 开头他号段
	public static boolean checkMobileStartNum(String mobile,String screenMobileStart){
		String[] mobileStarts = JSON.parseObject(screenMobileStart, String[].class);
		for(String mobileStart:mobileStarts){
			if(mobile.startsWith(mobileStart)){
				logger.warn("the mobile is blocked");
				return true;
			}
		}
		return false;
